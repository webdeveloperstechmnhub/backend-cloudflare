...
async function generateTicketPDF(user, event, registrationId) {
  try {
    ...
    const eventName = event?.name || "TechMNHub Event";
    const dateValue = event?.date ? new Date(event.date).toLocaleDateString("en-IN") : "Date TBD";
    const cityValue = event?.city || "Venue TBD";

    page.drawText("TechMNHub Ticket", {
      x: 48,
      y: 790,
      size: 24,
      font: bold,
      color: rgb(0.03, 0.57, 0.69),
    });
    ...
    const rows = [
      ...
      ["Date", dateValue],
      ["City", cityValue],
      ["Pass", user.passName || user.passType || "Standard"],
      ["Amount", `INR ${user.amountPaid || 0}`],
    ];

    let y = 748;
    for (const [label, value] of rows) {
      page.drawText(`${label}:`, { x: 48, y, size: 12, font: bold, color: rgb(0.15, 0.15, 0.15) });
      page.drawText(String(value), { x: 190, y, size: 12, font, color: rgb(0.2, 0.2, 0.2) });
      y -= 24;
    }

    page.drawText("Present this PDF and registration ID at check-in.", {
      x: 48,
      y: y - 16,
      size: 11,
      font,
